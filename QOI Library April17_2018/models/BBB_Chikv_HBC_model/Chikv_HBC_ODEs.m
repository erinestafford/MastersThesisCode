...
%% Vectors
S_v = init_conditions(7);
E_v = init_conditions(8);
I_v = init_conditions(9);
...
%vector
Y(7) = (psi_v - (psi_v - mu_v)*(N_v/K_v))*N_v - (lambda_v*S_v) - (mu_v * S_v);
Y(8) = (lambda_v* S_v) - (nu_v + mu_v) * E_v;
Y(9) = (nu_v*E_v) - (mu_v * I_v);
...
out(5) = Y(5);
out(6) = Y(6);
out(7) = Y(7); %susceptible vector
out(8) = Y(8); %exposed vector
out(9) = Y(9); %infected vector
