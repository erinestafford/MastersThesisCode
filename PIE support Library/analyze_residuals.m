...
function [str] = analyze_residuals(tdata,ydata,ydata_fit,residuals,str)
...
if str.verbose; disp('BEGIN RESIDUAL ANALYSIS TESTS');disp(' '); 
   ...
   % plot residuals versus time 
    for iy=1:nydim
        subplot(3,nydim,iy)
        plot(tdata,residuals(:,iy),'*');
        xlabel('time');ylabel(['residuals ',num2str(iy)]);
    end
    
     % plot residuals versus time 
    for iy=1:nydim
        subplot(3,nydim,nydim+iy)
        plot(ydata(:,iy),residuals(:,iy),'*');
         xlabel(['ydata ',num2str(iy)]);ylabel(['residuals ',num2str(iy)]);
    end
       
        
     % plot the data and fit versus time 
    for iy=1:nydim
        subplot(3,nydim,2*nydim+iy)
        plot(tdata,ydata(:,iy),'.');hold on;
        plot(tdata,ydata_fit(:,iy),'k');
         xlabel('time');ylabel(['ydata and ydata fit',num2str(iy)]);
%          legend([{'ydata '},{'ydata-fit'}])
    end
end
